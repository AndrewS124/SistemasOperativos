/*
 * Fecha: 29/04/2024
 * Autor: Santiago Chitiva Contreras, Martin Medina Novoa, Andres David Rueda
 * Sandoval Materia: Sistemas Operativos Tema: Proyecto de Curso Objetivo:
 ...
 *
 * Ejecucion:
 * 1. Este programa se ejecuta de esta manera: ./monitor -b 1 -t file-temp.txt -p pipe1
...
int verificar_argumentos(int argc, const char *argv[], Argumentos *args) {
  ...
  if (args->tam_buffer <= 0 || args->file_temp == NULL ||
      args->pipe_nominal == NULL) {
...
void *recolector(void *arg) {
  ...
  while (1) {
    ...
    if (strcmp(buf, "FIN") == 0) {
      printf(
          "Mensaje de finalización recibido. Terminando el hilo recolector.\n");
